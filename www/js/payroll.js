...
function sent (){
    ...
    $.ajax({
        ...
        data: { start: 'first day of january this year', end: 'tomorrow', org_id : oid, 'is_closed': 0},
        ...
        success: function(msg){
            if(msg.status==='success'){
                ...
                $(msg.data.data).each(function () {
                    ...
                    var html = '<div class="item sentReceipt"><div class="row"><div class="col"><p class="align-center" style="font-size:18px;padding:5px;"><strong>'+this.total+'</strong></p></div></div><div class="row"><div class="col"><p class="align-center">Dikirim tanggal<br/>'+d+' '+m+' '+Y+'</p></div><div class="col"><button class="green radius full btn-sm" onClick="onClosed('+this.id+')">Diterima</button></div></div></div>';
                    $('#sentSection').append(html); //append your new tr
                });
            }else{
                alert(msg.message);
...
function closed (){
    ...
    $.ajax({
        ...
        // http://en.wikipedia.org/wiki/Same_origin_policy
        url: window.localStorage.getItem('base_url')+"/hr/payroll",
        data: { start: 'first day of january this year', end: 'tomorrow', org_id : oid, 'is_closed': 1},
        beforeSend: function (xhr) {
            /* Authorization header */
            xhr.setRequestHeader("Authorization", "Bearer " + sessionStorage.getItem('user.jwt'));
            // xhr.setRequestHeader("X-Mobile", "false");
        ...
        success: function(msg){
            if(msg.status==='success'){
                ...
                $(msg.data.data).each(function () {
                    ...
                    var html = '<div class="item closedReceipt"><div class="row"><div class="col"><p class="align-center" style="font-size:18px;padding:5px;"><strong>'+this.total+'</strong></p></div></div><div class="row"><div class="col"><p class="align-center">Dikirim tanggal<br/>'+d+' '+m+' '+Y+'</p></div><div class="col"><p class="align-center">Diterima tanggal <br/>'+d2+' '+m2+' '+Y2+'</p></div></div></div>';
                    $('#closedSection').append(html); //append your new tr
                });
            }else{
                alert(msg.message);
